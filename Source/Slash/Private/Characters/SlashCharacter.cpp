...
{
	if (EquippedWeapon) EquippedWeapon->AttachToSocket(GetMesh(), FName("RightHandSocket"));
}

void ASlashCharacter::OnWeaponDisarm()
{
	if (EquippedWeapon) EquippedWeapon->AttachToSocket(GetMesh(), FName("SpineSocket"));
}
